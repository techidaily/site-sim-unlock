  ...
  check() {
    ...
    if (lastPublishDate) {
      ...
      if (diffHours < minHours) {
        console.log(`距离上一次发布不到${minHours}小时，忽略发布. 已经历时 ${diffHours} 小时`);
        return false;
      }
    }
...
class Url2TitleMapHelper {
  ...
  getShuffleUrl2TitleMap(url, maxCount = 15) {
    ...
    const shuffleKeys = shuffle(keys.filter(key => key !== filterKey)).slice(0, maxCount);
    ...
    const globalShuffleKeys = shuffle(globalKeys.filter(key => key !== filterKey && !key.startsWith(`${this.siteUrl}/`))).slice(0, globalCount);
...
const updatePostsTask = () => {
  ...
  const addAlsoReadContent = (url, content) => {
    ...
    const shuffleMap = urlMapHelper.getShuffleUrl2TitleMap(url, randomIntFromInterval(15, 30));
  ...
  // 任务1: 旧文章先更新
  const oldestPosts = posts.slice(0, posts.length < maxAllPostCount ? 100 : 200);
...
const publishTask = () => {
  ...
  try {
    ...
    if (stdout.toString().indexOf('HEAD -> gh-pages') === -1) {
      error = new Error('发布到 gh-pages 分支失败');
    }
  } catch (e) {
    console.log(`代码错误:\n`);
    console.error(e);
